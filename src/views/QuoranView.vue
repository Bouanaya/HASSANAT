  ...
  <div class="w-full h-screen bg-[#0F172A] ">
    <div class="w-full  md:pl-16 space-y-8 bg-[#0F172A] flex flex-col">
      <div class="text-primary-400 Search bg-[#0F172A]  flex justify-center fixed z-50 py-10">
        ...
        <div class="absolute w-[90%] m-auto h-10 bg-[#0F172A] blur-sm -bottom-4"></div>

      </div>

      <div class="md:pl-24 px-10 flex flex-wrap gap-8 py-24" v-if="dataLoaded">
        <RouterLink :to='`/Quoran/${item.englishName}`' v-for="item in Qur " :key="item.number" class="duration-500   relative w-48 hover:bg-primary-300 hover:shadow-lg hover:shadow-primary-500/30 
  ...
  onMounted(() => {
    axios.get('http://api.alquran.cloud/v1/quran/ar.alafasy')
      .then(res => {
        ...
        dataLoaded.value = true; // Set to true when data is ready
      })
      .catch(error => {
